...
def run_import():
    ...
    try:
        ...
        if os.path.exists(path_users):
            ...
            for _, row in df.iterrows():
                role_id = get_or_create_id(cursor, "roles", row["Роль сотрудника"])
                cursor.execute(
                    "INSERT OR IGNORE INTO users (login, password, fio, role_id) VALUES (?,?,?,?)",
                    (str(row["Логин"]), str(row["Пароль"]), str(row["ФИО"]), role_id),
                )
        ...
        if os.path.exists(path_tovar):
            ...
            for _, row in df.iterrows():
                cat_id = get_or_create_id(cursor, "categories", row["Категория товара"])
                man_id = get_or_create_id(cursor, "manufacturers", row["Производитель"])
                sup_id = get_or_create_id(cursor, "suppliers", row["Поставщик"])
                unit_id = get_or_create_id(cursor, "units", row["Единица измерения"])
